...
export default function AddUserModal({
  ...
  const handleLoadUserReferences = useCallback(async () => {
    try {
      ...
      if (status !== 200) {
        console.error(
          "Unexpected status error during load user references at AddUserModal.tsx: ",
          status
        );
        return;
      }
      ...
      console.error(
        "Unexpected server error during load user references at AddUserModal.tsx: ",
        error
      );
    }
  }, []);
  ...
  const handleStoreUser = async (e: FormEvent) => {
    try {
      ...
      if (status !== 200) {
        console.error(
          "Unexpected status error during add user at AddUserModal.tsx: ",
          status
        );
        return;
      }

      onUserAdded("success", data.message);
      onReloadUserReferences();
      ...
      if (error.response && error.response.status !== 422) {
        console.error(
          "Unexpected server error during add user at AddUserModal.tsx: ",
          error
        );
        return;
      }
